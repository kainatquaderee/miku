...
export default class App {
  ...
  constructor () {
    ...
    if (process.platform === 'win32') {
      app.setAppUserModelId('com.github.kainatquaderee.miku')
    ...
    this.mainWindow.webContents.on('render-process-gone', async (e, { reason }) => {
      if (reason === 'crashed') {
        if (++crashcount > 10) {
          await dialog.showMessageBox({ message: 'Crashed too many times.', title: 'Miku', detail: 'App crashed too many times. For a fix visit https://miku.watch/faq/', icon: '/renderer/public/logo_filled.png' })
          shell.openExternal('https://miku.watch/faq/')
        } else {
          app.relaunch()
        }
        app.quit()
